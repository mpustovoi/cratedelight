...
public class SonicBoom extends Behavior<Warden> {
    ...
    protected void tick(ServerLevel pLevel, Warden pOwner, long pGameTime) {
        pOwner.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).ifPresent(p_352767_ -> pOwner.getLookControl().setLookAt(p_352767_.position()));
        if (!pOwner.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY)
            ...
            pOwner.getBrain()
                ...
                .ifPresent(p_340767_ -> {
                    ...
                    if (p_340767_.hurt(pLevel.damageSources().sonicBoom(pOwner), 10.0F)) {
