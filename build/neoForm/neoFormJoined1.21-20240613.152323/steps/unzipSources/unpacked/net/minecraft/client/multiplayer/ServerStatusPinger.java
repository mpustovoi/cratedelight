        ...
        if (optional.isEmpty()) {
            ...
            ClientStatusPacketListener clientstatuspacketlistener = new ClientStatusPacketListener() {
                ...
                public void handleStatusResponse(ClientboundStatusResponsePacket p_105489_) {
                    if (this.receivedPing) {
                        ...
                        serverstatus.favicon().ifPresent(p_302312_ -> {
                            if (!Arrays.equals(p_302312_.iconBytes(), pServerData.getIconBytes())) {
                                pServerData.setIconBytes(ServerData.validateIcon(p_302312_.iconBytes()));
                ...
                public void onDisconnect(DisconnectionDetails p_350468_) {
                    if (!this.success) {
                        ServerStatusPinger.this.onPingFailed(p_350468_.reason(), pServerData);
    ...
    void pingLegacyServer(InetSocketAddress pResolvedServerAddress, final ServerAddress pServerAddress, final ServerData pServerData) {
        new Bootstrap().group(Connection.NETWORK_WORKER_GROUP.get()).handler(new ChannelInitializer<Channel>() {
            ...
            protected void initChannel(Channel p_105498_) {
                ...
                p_105498_.pipeline().addLast(new LegacyServerPinger(pServerAddress, (p_315832_, p_315833_, p_315834_, p_315835_, p_315836_) -> {
                    pServerData.setState(ServerData.State.INCOMPATIBLE);
                    pServerData.version = Component.literal(p_315833_);
                    pServerData.motd = Component.literal(p_315834_);
                    pServerData.status = ServerStatusPinger.formatPlayerCount(p_315835_, p_315836_);
                    pServerData.players = new ServerStatus.Players(p_315836_, p_315835_, List.of());
                }));
            }
