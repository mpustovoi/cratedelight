...
public class ShaderInstance implements Shader, AutoCloseable {
    ...
    private static Program getOrCreate(final ResourceProvider pResourceProvider, Program.Type pProgramType, String pName) throws IOException {
        ...
        Program program;
        if (program1 == null) {
            ResourceLocation loc = ResourceLocation.parse(pName);
    ...
    public void setDefaultUniforms(VertexFormat.Mode pMode, Matrix4f pProjectionMatrix, Matrix4f pFrustrumMatrix, Window pWindow) {
        for (int i = 0; i < 12; i++) {
        ...
        if (this.MODEL_VIEW_MATRIX != null) {
            this.MODEL_VIEW_MATRIX.set(pProjectionMatrix);
        }

        if (this.PROJECTION_MATRIX != null) {
            this.PROJECTION_MATRIX.set(pFrustrumMatrix);
        }
