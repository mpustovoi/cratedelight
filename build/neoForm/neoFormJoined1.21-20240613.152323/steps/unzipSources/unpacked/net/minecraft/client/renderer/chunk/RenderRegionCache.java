...
public class RenderRegionCache {
    ...
    public RenderChunkRegion createRegion(Level pLevel, SectionPos pSectionPos) {
        return createRegion(pLevel, pSectionPos, true);
    }

    @Nullable
    public RenderChunkRegion createRegion(Level pLevel, SectionPos pSectionPos, boolean nullForEmpty) {
        RenderRegionCache.ChunkInfo renderregioncache$chunkinfo = this.getChunkInfo(pLevel, pSectionPos.x(), pSectionPos.z());
        if (nullForEmpty && renderregioncache$chunkinfo.chunk().isSectionEmpty(pSectionPos.y())) {
            return null;
        } else {
            ...
            int sectionMinY = pSectionPos.getY() - RenderChunkRegion.RADIUS;
            int sectionMaxY = pSectionPos.getY() + RenderChunkRegion.RADIUS;
            var modelDataManager = pLevel.getModelDataManager().snapshotSectionRegion(i, sectionMinY, j, k, sectionMaxY, l);
            return new RenderChunkRegion(pLevel, i, j, arenderchunk, modelDataManager);
        }
    }
