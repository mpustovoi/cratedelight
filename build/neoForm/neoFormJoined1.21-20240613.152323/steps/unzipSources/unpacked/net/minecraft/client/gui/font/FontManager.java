...
public class FontManager implements PreparableReloadListener, AutoCloseable {
    ...
    public FontManager(TextureManager pTextureManager) {
        ...
        this.missingFontSet = Util.make(new FontSet(pTextureManager, MISSING_FONT), p_325488_ -> p_325488_.reload(List.of(createFallbackProvider()), Set.of()));
    }
            ...
            list.add(CompletableFuture.supplyAsync(() -> {
                ...
                for (Pair<FontManager.BuilderId, GlyphProviderDefinition.Conditional> pair : list1) {
                    ...
                    pair.getSecond().definition().unpack().ifLeft(p_325476_ -> {
                        CompletableFuture<Optional<GlyphProvider>> completablefuture = this.safeLoad(fontmanager$builderid, p_325476_, pResourceManager, pExecutor);
                                ...
                                CompletableFuture<?>[] completablefuture = map.values()
                                    ...
                                    .map(
                                        p_284585_ -> CompletableFuture.runAsync(
                                                () -> this.finalizeProviderLoading(p_284585_, glyphprovider$conditional), pExecutor
                                            )
                                    )
    ...
    private void apply(FontManager.Preparation pPreperation, ProfilerFiller pProfiler) {
        ...
        pPreperation.fontSets().forEach((p_325478_, p_325479_) -> {
    ...
    private FontSet getFontSetCached(ResourceLocation p_326503_) {
        ...
        if (fontset != null && p_326503_.equals(fontset.name())) {
            ...
            FontSet fontset1 = this.getFontSetRaw(p_326503_);
        ...
        public Optional<List<GlyphProvider.Conditional>> resolve(Function<ResourceLocation, List<GlyphProvider.Conditional>> pProviderResolver) {
            return this.result
                .map(
                    ...
                    p_325490_ -> {
                        List<GlyphProvider.Conditional> list = pProviderResolver.apply(p_325490_);
                        if (list == null) {
                            FontManager.LOGGER
                                .warn(
