...
public interface ByteBufCodecs {
    ...
    static StreamCodec<ByteBuf, byte[]> byteArray(final int pMaxSize) {
        return new StreamCodec<ByteBuf, byte[]>() {
            public byte[] decode(ByteBuf p_319947_) {
                return FriendlyByteBuf.readByteArray(p_319947_, pMaxSize);
            }

            public void encode(ByteBuf p_320370_, byte[] p_331189_) {
                if (p_331189_.length > pMaxSize) {
                    throw new EncoderException("ByteArray with size " + p_331189_.length + " is bigger than allowed " + pMaxSize);
                } else {
    ...
    static StreamCodec<ByteBuf, String> stringUtf8(final int pMaxLength) {
        return new StreamCodec<ByteBuf, String>() {
            public String decode(ByteBuf p_332176_) {
                return Utf8String.read(p_332176_, pMaxLength);
            }

            public void encode(ByteBuf p_331068_, String p_341104_) {
                Utf8String.write(p_331068_, p_341104_, pMaxLength);
            }
        };
    }
    ...
    static StreamCodec<ByteBuf, Tag> tagCodec(final Supplier<NbtAccounter> pAccounter) {
        return new StreamCodec<ByteBuf, Tag>() {
            public Tag decode(ByteBuf p_341393_) {
                Tag tag = FriendlyByteBuf.readNbt(p_341393_, pAccounter.get());
                if (tag == null) {
    ...
    static StreamCodec<ByteBuf, CompoundTag> compoundTagCodec(Supplier<NbtAccounter> pAccounterSupplier) {
        return tagCodec(pAccounterSupplier).map(p_339405_ -> {
    ...
    static <T> StreamCodec<ByteBuf, T> fromCodec(Codec<T> pCodec, Supplier<NbtAccounter> pAccounterSupplier) {
        return tagCodec(pAccounterSupplier)
            .map(
                p_337514_ -> pCodec.parse(NbtOps.INSTANCE, p_337514_)
                        .getOrThrow(p_339407_ -> new DecoderException("Failed to decode: " + p_339407_ + " " + p_337514_)),
                p_337516_ -> pCodec.encodeStart(NbtOps.INSTANCE, (T)p_337516_)
    ...
    static <T> StreamCodec<RegistryFriendlyByteBuf, T> fromCodecWithRegistries(final Codec<T> pCodec, Supplier<NbtAccounter> pAccounterSupplier) {
        ...
        return new StreamCodec<RegistryFriendlyByteBuf, T>() {
            public T decode(RegistryFriendlyByteBuf p_340878_) {
                ...
                return pCodec.parse(registryops, tag).getOrThrow(p_340924_ -> new DecoderException("Failed to decode: " + p_340924_ + " " + tag));
            }
            ...
            public void encode(RegistryFriendlyByteBuf p_341221_, T p_341320_) {
                ...
                Tag tag = pCodec.encodeStart(registryops, p_341320_)
    ...
    static <B extends ByteBuf, V> StreamCodec<B, Optional<V>> optional(final StreamCodec<B, V> pCodec) {
        return new StreamCodec<B, Optional<V>>() {
            public Optional<V> decode(B p_324595_) {
                return p_324595_.readBoolean() ? Optional.of(pCodec.decode(p_324595_)) : Optional.empty();
            }

            public void encode(B p_324147_, Optional<V> p_340875_) {
                if (p_340875_.isPresent()) {
                    p_324147_.writeBoolean(true);
                    pCodec.encode(p_324147_, p_340875_.get());
                } else {
    ...
    static <B extends ByteBuf, V, C extends Collection<V>> StreamCodec<B, C> collection(
        ...
        return new StreamCodec<B, C>() {
            public C decode(B p_324220_) {
                int i = ByteBufCodecs.readCount(p_324220_, pMaxSize);
                C c = pFactory.apply(Math.min(i, 65536));

                for (int j = 0; j < i; j++) {
                    c.add(pCodec.decode(p_324220_));
                }
            ...
            public void encode(B p_323874_, C p_340813_) {
                ByteBufCodecs.writeCount(p_323874_, p_340813_.size(), pMaxSize);

                for (V v : p_340813_) {
                    pCodec.encode(p_323874_, v);
                }
            }
        };
    }
    ...
    static <B extends ByteBuf, V, C extends Collection<V>> StreamCodec.CodecOperation<B, V, C> collection(IntFunction<C> pFactory) {
        return p_319785_ -> collection(pFactory, p_319785_);
    }
    ...
    static <B extends ByteBuf, V> StreamCodec.CodecOperation<B, V, List<V>> list(int pMaxSize) {
        return p_329871_ -> collection(ArrayList::new, p_329871_, pMaxSize);
    }
    ...
    static <B extends ByteBuf, K, V, M extends Map<K, V>> StreamCodec<B, M> map(
        ...
        return new StreamCodec<B, M>() {
            public void encode(B p_331539_, M p_341314_) {
                ByteBufCodecs.writeCount(p_331539_, p_341314_.size(), pMaxSize);
                p_341314_.forEach((p_340647_, p_340648_) -> {
                    pKeyCodec.encode(p_331539_, (K)p_340647_);
                    pValueCodec.encode(p_331539_, (V)p_340648_);
                });
            }

            public M decode(B p_331901_) {
                int i = ByteBufCodecs.readCount(p_331901_, pMaxSize);
                M m = (M)pFactory.apply(Math.min(i, 65536));

                for (int j = 0; j < i; j++) {
                    K k = pKeyCodec.decode(p_331901_);
                    V v = pValueCodec.decode(p_331901_);
                    m.put(k, v);
                }
    ...
    static <B extends ByteBuf, L, R> StreamCodec<B, Either<L, R>> either(final StreamCodec<? super B, L> pLeftCodec, final StreamCodec<? super B, R> pRightCodec) {
        return new StreamCodec<B, Either<L, R>>() {
            public Either<L, R> decode(B p_332082_) {
                return p_332082_.readBoolean() ? Either.left(pLeftCodec.decode(p_332082_)) : Either.right(pRightCodec.decode(p_332082_));
            }

            public void encode(B p_331172_, Either<L, R> p_340944_) {
                p_340944_.ifLeft(p_341317_ -> {
                    p_331172_.writeBoolean(true);
                    pLeftCodec.encode(p_331172_, (L)p_341317_);
                }).ifRight(p_341155_ -> {
                    p_331172_.writeBoolean(false);
                    pRightCodec.encode(p_331172_, (R)p_341155_);
                });
            }
        };
    }
    ...
    static <T> StreamCodec<ByteBuf, T> idMapper(final IntFunction<T> pIdLookup, final ToIntFunction<T> pIdGetter) {
        return new StreamCodec<ByteBuf, T>() {
            ...
            public void encode(ByteBuf p_341417_, T p_330257_) {
                int i = pIdGetter.applyAsInt(p_330257_);
    ...
    private static <T, R> StreamCodec<RegistryFriendlyByteBuf, R> registry(
        ...
        return new StreamCodec<RegistryFriendlyByteBuf, R>() {
            private IdMap<R> getRegistryOrThrow(RegistryFriendlyByteBuf p_330361_) {
                var registry = p_330361_.registryAccess().registryOrThrow(pRegistryKey);
                ...
                return pIdGetter.apply(registry);
            }
    ...
    static <T> StreamCodec<RegistryFriendlyByteBuf, T> registry(ResourceKey<? extends Registry<T>> pRegistryKey) {
        return registry(pRegistryKey, p_332056_ -> p_332056_);
    }
    ...
    static <T> StreamCodec<RegistryFriendlyByteBuf, Holder<T>> holder(
        ...
        return new StreamCodec<RegistryFriendlyByteBuf, Holder<T>>() {
            ...
            private IdMap<Holder<T>> getRegistryOrThrow(RegistryFriendlyByteBuf p_341377_) {
                return p_341377_.registryAccess().registryOrThrow(pRegistryKey).asHolderIdMap();
            }

            public Holder<T> decode(RegistryFriendlyByteBuf p_330998_) {
                int i = VarInt.read(p_330998_);
                return i == 0 ? Holder.direct(pCodec.decode(p_330998_)) : (Holder)this.getRegistryOrThrow(p_330998_).byIdOrThrow(i - 1);
            }
            ...
            public void encode(RegistryFriendlyByteBuf p_330557_, Holder<T> p_341109_) {
                switch (p_341109_.kind()) {
                    ...
                    case DIRECT:
                        ...
                        pCodec.encode(p_330557_, p_341109_.value());
                }
            }
        };
    }
    ...
    static <T> StreamCodec<RegistryFriendlyByteBuf, HolderSet<T>> holderSet(final ResourceKey<? extends Registry<T>> pRegistryKey) {
        return new StreamCodec<RegistryFriendlyByteBuf, HolderSet<T>>() {
            ...
            private StreamCodec<RegistryFriendlyByteBuf, ? extends net.neoforged.neoforge.registries.holdersets.ICustomHolderSet<T>> holderSetCodec(net.neoforged.neoforge.registries.holdersets.HolderSetType type) {
                return this.holderSetCodecs.computeIfAbsent(type, key -> key.makeStreamCodec(pRegistryKey));
            }
            ...
            public HolderSet<T> decode(RegistryFriendlyByteBuf p_340887_) {
                ...
                if (i == -1) {
                    Registry<T> registry = p_340887_.registryAccess().registryOrThrow(pRegistryKey);
                    return registry.getTag(TagKey.create(pRegistryKey, ResourceLocation.STREAM_CODEC.decode(p_340887_))).orElseThrow();
                } else {
