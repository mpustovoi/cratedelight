...
public class ItemEntity extends Entity implements TraceableEntity {
    ...
    public void playerTouch(Player pEntity) {
        if (!this.level().isClientSide) {
            ...
            var result = net.neoforged.neoforge.event.EventHooks.fireItemPickupPre(this, pEntity).canPickup();
            if (result.isFalse()) {
                return;
            }
            ...
            if ((result.isTrue() || this.pickupDelay == 0 && (this.target == null || this.target.equals(pEntity.getUUID()))) && pEntity.getInventory().add(itemstack)) {
                // Fire ItemEntityPickupEvent.Post
                net.neoforged.neoforge.event.EventHooks.fireItemPickupPost(this, pEntity, originalCopy);
