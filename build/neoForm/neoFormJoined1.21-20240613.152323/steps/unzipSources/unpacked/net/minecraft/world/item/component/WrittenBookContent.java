...
public record WrittenBookContent(Filterable<String> title, String author, int generation, List<Filterable<Component>> pages, boolean resolved)
    ...
    private static Optional<Filterable<Component>> resolvePage(CommandSourceStack pSource, @Nullable Player pPlayer, Filterable<Component> pPages) {
        return pPages.resolve(p_331526_ -> {
            try {
                Component component = ComponentUtils.updateForEntity(pSource, p_331526_, pPlayer, 0);
    ...
    public List<Component> getPages(boolean pFiltered) {
        return Lists.transform(this.pages, p_332134_ -> p_332134_.get(pFiltered));
    }
