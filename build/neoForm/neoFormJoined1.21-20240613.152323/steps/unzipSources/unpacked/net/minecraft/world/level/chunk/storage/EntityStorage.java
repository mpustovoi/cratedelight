...
public class EntityStorage implements EntityPersistentStorage<Entity> {
    ...
    private void reportSaveFailureIfPresent(CompletableFuture<?> pFuture, ChunkPos pPos) {
        pFuture.exceptionally(p_351986_ -> {
            LOGGER.error("Failed to store entity chunk {}", pPos, p_351986_);
            this.level.getServer().reportChunkSaveFailure(p_351986_, this.simpleRegionStorage.storageInfo(), pPos);
            return null;
        });
    }

    private void reportLoadFailureIfPresent(CompletableFuture<?> pFuture, ChunkPos pPos) {
        pFuture.exceptionally(p_351990_ -> {
            LOGGER.error("Failed to load entity chunk {}", pPos, p_351990_);
            this.level.getServer().reportChunkLoadFailure(p_351990_, this.simpleRegionStorage.storageInfo(), pPos);
            return null;
        });
    }
