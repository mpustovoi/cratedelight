        ...
        default HolderLookup.RegistryLookup<T> filterElements(final Predicate<T> pPredicate) {
            return new HolderLookup.RegistryLookup.Delegate<T>() {
                ...
                public Optional<Holder.Reference<T>> get(ResourceKey<T> p_323784_) {
                    return this.parent().get(p_323784_).filter(p_324360_ -> pPredicate.test(p_324360_.value()));
                }

                @Override
                public Stream<Holder.Reference<T>> listElements() {
                    return this.parent().listElements().filter(p_324273_ -> pPredicate.test(p_324273_.value()));
                }
            };
        }
