...
public class ExtraCodecs {
    ...
    public static <T> Codec<T> converter(DynamicOps<T> pOps) {
        return Codec.PASSTHROUGH.xmap(p_304323_ -> p_304323_.convert(pOps).getValue(), p_304327_ -> new Dynamic<>(pOps, (T)p_304327_));
    }
    ...
    public static <P, I> Codec<I> intervalCodec(
        ...
        Codec<I> codec = Codec.list(pCodec).comapFlatMap(p_337577_ -> Util.fixedSize((List<P>)p_337577_, 2).flatMap(p_184445_ -> {
                P p = p_184445_.get(0);
    ...
    public static <K, V> Codec<Map<K, V>> sizeLimitedMap(Codec<Map<K, V>> pMapCodec, int pMaxSize) {
        return pMapCodec.validate(
            p_329967_ -> p_329967_.size() > pMaxSize
                    ? DataResult.error(() -> "Map is too long: " + p_329967_.size() + ", expected range [0-" + pMaxSize + "]")
        ...
        return new MapCodec<V>() {
            ...
            public <T> DataResult<V> decode(DynamicOps<T> p_312013_, MapLike<T> p_312527_) {
                ...
                return t == null
                    ...
                    : pCodec.decode(p_312013_, t).flatMap(p_337590_ -> {
                        T t1 = Objects.requireNonNullElseGet(p_312527_.get(p_312135_), p_312013_::emptyMap);
                        return pCodecGetter.apply(p_337590_.getFirst()).decode(p_312013_, t1).map(Pair::getFirst);
                    });
            }

            @Override
            public <T> RecordBuilder<T> encode(V p_312741_, DynamicOps<T> p_312649_, RecordBuilder<T> p_312297_) {
                K k = (K)pKeyGetter.apply(p_312741_);
                p_312297_.add(p_312812_, pCodec.encodeStart(p_312649_, k));
                DataResult<T> dataresult = this.encode((Codec<? extends V>)pCodecGetter.apply(k), p_312741_, p_312649_);
    ...
    public static record StrictUnboundedMapCodec<K, V>(Codec<K> keyCodec, Codec<V> elementCodec) implements Codec<Map<K, V>>, BaseMapCodec<K, V> {
        ...
        public <T> DataResult<Pair<Map<K, V>, T>> decode(DynamicOps<T> pOps, T pInput) {
            return pOps.getMap(pInput)
                ...
                .flatMap(p_301208_ -> this.decode(pOps, (MapLike<T>)p_301208_))
                .map(p_300941_ -> Pair.of((Map<K, V>)p_300941_, pInput));
        }
