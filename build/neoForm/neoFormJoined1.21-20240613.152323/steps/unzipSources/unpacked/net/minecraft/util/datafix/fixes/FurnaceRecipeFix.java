...
public class FurnaceRecipeFix extends DataFix {
    ...
    private <R> Typed<?> updateFurnaceContents(
        Type<R> pType, Type<Pair<Either<Pair<List<Pair<R, Integer>>, Dynamic<?>>, Unit>, Dynamic<?>>> pRecipesUsed, Typed<?> pData
    ) {
        ...
        for (int j = 0; j < i; j++) {
            ...
            if (k > 0) {
                optional.ifPresent(p_337634_ -> {
                    Optional<? extends Pair<R, ? extends Dynamic<?>>> optional1 = pType.read((Dynamic<?>)p_337634_).result();
