    ...
    public static <T, C, P> Grammar<List<T>> createGrammar(ComponentPredicateParser.Context<T, C, P> pContext) {
        ...
        dictionary.put(
            atom5, Term.sequence(Term.named(atom6), Term.optional(Term.sequence(StringReaderTerms.character(','), Term.named(atom5)))), p_336093_ -> {
                T t = pContext.anyOf(p_336093_.getOrThrow(atom6));
        ...
        dictionary.put(atom8, Term.named(atom9), p_335793_ -> pContext.negate(p_335793_.getOrThrow(atom9)));
        dictionary.put(
            atom9,
            Term.alternative(
            ...
            (p_335570_, p_336062_) -> {
                ...
                try {
                    if (p != null) {
                        ...
                        return Optional.of(pContext.createPredicateTest(p_335570_.input(), p, tag1));
                    } else {
                        C c = p_336062_.getOrThrow(atom10);
                        Tag tag = p_336062_.get(atom13);
                        return Optional.of(
                            tag != null ? pContext.createComponentTest(p_335570_.input(), c, tag) : pContext.createComponentTest(p_335570_.input(), c)
                        );
                    }
