...
public abstract class AgeableListModel<E extends Entity> extends EntityModel<E> {
    ...
    public void renderToBuffer(PoseStack pPoseStack, VertexConsumer pBuffer, int pPackedLight, int pPackedOverlay, int pColor) {
        if (this.young) {
            ...
            this.headParts().forEach(p_349807_ -> p_349807_.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pColor));
            pPoseStack.popPose();
            pPoseStack.pushPose();
            float f1 = 1.0F / this.babyBodyScale;
            pPoseStack.scale(f1, f1, f1);
            pPoseStack.translate(0.0F, this.bodyYOffset / 16.0F, 0.0F);
            this.bodyParts().forEach(p_349825_ -> p_349825_.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pColor));
            pPoseStack.popPose();
        } else {
            this.headParts().forEach(p_349819_ -> p_349819_.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pColor));
            this.bodyParts().forEach(p_349813_ -> p_349813_.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pColor));
        }
    }
