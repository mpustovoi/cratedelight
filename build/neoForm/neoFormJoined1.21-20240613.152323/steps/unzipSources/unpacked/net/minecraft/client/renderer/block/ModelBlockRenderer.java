...
public class ModelBlockRenderer {
    ...
    public void tesselateBlock(
        ...
        boolean flag = Minecraft.useAmbientOcclusion() && switch(pModel.useAmbientOcclusion(pState, modelData, renderType)) {
            case TRUE -> true;
            case DEFAULT -> pState.getLightEmission(pLevel, pPos) == 0;
            case FALSE -> false;
        };
    ...
    public void tesselateWithAO(
            ...
            if (!list.isEmpty()) {
                ...
                if (!pCheckSides || Block.shouldRenderFace(pState, pLevel, pPos, direction, blockpos$mutableblockpos)) {
                    this.renderModelFaceAO(
                        pLevel, pState, pPos, pPoseStack, pConsumer, list, afloat, bitset, modelblockrenderer$ambientocclusionface, pPackedOverlay
                    );
                }
            }
        }
        ...
        pRandom.setSeed(pSeed);
        List<BakedQuad> list1 = pModel.getQuads(pState, (Direction)null, pRandom, modelData, renderType);
        if (!list1.isEmpty()) {
            this.renderModelFaceAO(
                pLevel, pState, pPos, pPoseStack, pConsumer, list1, afloat, bitset, modelblockrenderer$ambientocclusionface, pPackedOverlay
            );
        }
    }
    ...
    public void tesselateWithoutAO(
        ...
        pRandom.setSeed(pSeed);
        List<BakedQuad> list1 = pModel.getQuads(pState, null, pRandom, modelData, renderType);
        if (!list1.isEmpty()) {
