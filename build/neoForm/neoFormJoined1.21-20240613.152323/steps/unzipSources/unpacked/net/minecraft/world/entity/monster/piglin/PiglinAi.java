...
public class PiglinAi {
    ...
    public static void angerNearbyPiglins(Player pPlayer, boolean pAngerOnlyIfCanSee) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !pAngerOnlyIfCanSee || BehaviorUtils.canSee(p_34881_, pPlayer)).forEach(p_352819_ -> {
            if (p_352819_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(p_352819_, pPlayer);
            } else {
                setAngerTarget(p_352819_, pPlayer);
            }
        });
    }
    ...
    protected static void wasHurtBy(Piglin pPiglin, LivingEntity pTarget) {
        if (!(pTarget instanceof Piglin)) {
            ...
            getAvoidTarget(pPiglin).ifPresent(p_348319_ -> {
                if (p_348319_.getType() != pTarget.getType()) {
    ...
    protected static void broadcastAngerTarget(AbstractPiglin pPiglin, LivingEntity pTarget) {
        getAdultPiglins(pPiglin).forEach(p_348314_ -> {
            if (pTarget.getType() != EntityType.HOGLIN || p_348314_.canHunt() && ((Hoglin)pTarget).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(p_348314_, pTarget);
            }
        });
    }
